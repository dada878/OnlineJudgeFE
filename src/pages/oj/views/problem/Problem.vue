    ...
    <div id="problem-main">
      ...
      <Panel :padding="40" shadow>
        <div slot="title" style="display: flex; align-items: center; gap: 12px;">
          {{ problem.title }}
          <Tag v-if="myStatusTag" :color="myStatusTag.color" style="margin-right: 12px;">
            {{$t('m.' + myStatusTag.name.replace(/ /g, '_'))}}
          </Tag>
        </div>
    ...
    methods: {
      ...
      init () {
        ...
        api[func](this.problemID, this.contestID).then(res => {
          ...
          // move python to the first place if it exists
          if (problem.languages.includes('Python3')) {
            problem.languages.unshift(problem.languages.splice(problem.languages.indexOf('Python3'), 1)[0])
          }
          // move c++ to the first place if it exists
          if (problem.languages.includes('C++')) {
            problem.languages.unshift(problem.languages.splice(problem.languages.indexOf('C++'), 1)[0])
          }
      ...
      fetchMyBestStatus () {
        ...
        // 選擇正確的 API
        const apiFunc = contestID ? api.getContestSubmissionList : api.getSubmissionList
        apiFunc(0, 100, params).then(res => {
          const results = res.data.data.results
          if (results && results.length > 0) {
            let bestSubmission = results[0]
...
<style lang="less" scoped>
  ...
  #info {
    ...
    ul {
      ...
      li {
        border-bottom: 1px dotted #156DD7;
        margin-bottom: 10px;
        p {
          display: inline-block;
        }
